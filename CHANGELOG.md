...
- Native/N-API engine failures now throw `AiError` (via `fromNativeError`) instead of raw `Error` — codes include `DEF_HASH_MISMATCH`, `GRAPH_ALREADY_REGISTERED`, `TOOL_ALREADY_REGISTERED`, `ENGINE_ERROR` (#46).
